...
for di = 1:length(Problist) 
    ...
    if abip
        ...
        data.c = full(c); 
        data.b = full(b);
        params_abip = struct('eps', 1e-3, 'max_admm_iters', 1000000, 'alpha', 1.8, 'adaptive_lookback', 10, 'sparsity_ratio', sp, 'verbose', 0);
    ...
    if scs
        ...
        tic; 
        [~, y, ~, info_scs] = scs_indirect(data, K, params_scs); 
        time_scs = toc; 
